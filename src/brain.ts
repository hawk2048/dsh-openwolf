/**
 * The brain directory: a durable, per-workspace store of context state that
 * survives sessions — session digests, learned preferences, action logs, bug
 * memory, and token accounting. Independent implementation of the OpenWolf
 * `.wolf/` concept for DeepSeek Harness (MIT; the reference is AGPL-3.0 and
 * is used only as a behavioral spec).
 *
 * The directory is `.dshwolf/` — deliberately NOT `.wolf/`, so the original
 * OpenWolf (for Claude Code / Codex / etc.) can manage the same workspace
 * without the two tools overwriting each other's state. Each tool keeps its
 * own brain; if you previously used this plugin's old `.wolf/` location,
 * `mv .wolf .dshwolf` migrates it in one step.
 *
...
/** The brain directory name under a workspace root. */
export const DEFAULT_BRAIN_DIR = '.dshwolf'
...
export class WolfBrain {
  ...
  constructor(root: string, dirName = DEFAULT_BRAIN_DIR) {
    this.root = root
...
`
  ...
  /** Render the map as the human-readable `.dshwolf/anatomy.md` view. */
  ...
  /**
   * Sync `.dshwolf/anatomy.md` to the current map. If a human edited the file
  ...
  /**
   * Persist the per-file anatomy index (`.dshwolf/anatomy-index.json`) so a
  ...
  /**
   * Run `fn` under a cross-process exclusive lock (`.dshwolf/.lock`, exclusive
